...
import numpy as np
from eval_dataset.RS_val_dataset import DataCollector, RRSISDDataset, ReasonSegDataset, RefSegRSDataset, RSRSDataset
...
from unigeoseg.model.builder import load_pretrained_model
from unigeoseg.utils import disable_torch_init
from unigeoseg.mm_utils import get_model_name_from_path
...
def compute_metric(intersection_meter, union_meter, acc_iou_meter, pr_meters, cur_res, gt, rsvg=False):  # 新增rsvg参数
    ...
    for i, result in enumerate(cur_res):
        ...
        for pred in preds_to_eval:
            # 若开启rsvg，将预测掩码转换为外接矩形掩码
            if rsvg:
                pred = get_bounding_box_mask(pred)  # 应用外接矩形转换
...
def get_largest_connected_component(mask):
    ...
    # 只保留最大连通区域
    largest_component = (labels == max_label).astype(np.uint8)
    return torch.from_numpy(largest_component).to(mask.device)

# 修改外接矩形生成函数，先获取最大连通区域
def get_bounding_box_mask(mask):
    """
    从二值掩码生成最大连通区域的外接矩形掩码
    mask: 输入二值掩码 (H, W)，0为背景，1为目标
    return: 矩形掩码 (H, W)，目标区域为最大连通区域的外接框
    """
    # 先获取最大连通区域
    largest_mask = get_largest_connected_component(mask)
    
    # 找到目标像素坐标
    coords = torch.nonzero(largest_mask)
    if len(coords) == 0:
        return mask  # 无目标时返回原掩码
...
class DataArguments:
    ...
    mask_config: Optional[str] = field(default="./unigeoseg/mask_config/maskformer2_swin_large.yaml")
    image_aspect_ratio: str = 'square'
    image_grid_pinpoints: Optional[str] = field(default=None)
    model_map_name: str = 'unigeoseg'
    ...
    rsvg: bool = field(default=False, metadata={"help": "Whether to use bounding box mask for IoU calculation"})
...
def evaluation():
    ...
    # 打印参数量（转换为百万/亿单位便于阅读）
    print(f"模型总参数量: {total_params / 1e6:.2f} M ({total_params / 1e9:.2f} B)")
    print(f"可训练参数量: {trainable_params / 1e6:.2f} M ({trainable_params / 1e9:.2f} B)")
    ...
    if data_args.dataset_type == 'RSRS':
        eval_dataset = RSRSDataset(
            base_data_path=data_args.base_data_path,
            image_size=512,
            tokenizer=tokenizer,
            split=data_args.data_split,
        ...
        for idx, inputs in tqdm(enumerate(eval_dataloader), total=len(eval_dataloader)):
            ...
            if 'token_answer_id' in inputs:
                ...
                outputs = model.eval_seg(
                    ...
                    prompt_type=inputs['prompt_type'] if 'prompt_type' in inputs else None,
                    intera_coords=inputs['intera_coords'] if 'intera_coords' in inputs else None
                    )
            else:
                outputs = model.eval_seg(
                    input_ids=inputs['input_ids'],
            ...
            compute_metric(intersection_meter,union_meter,acc_iou_meter, pr_meters, outputs, gt, rsvg=data_args.rsvg)
