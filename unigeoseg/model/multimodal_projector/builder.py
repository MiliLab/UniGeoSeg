...
class Compression_Connector(nn.Module):
    def __init__(self, c1=1024, c2=2048, patch_size=3, stride=2, size=16):
        ...
        self.proj1 = nn.Conv2d(c1, c2, patch_size, 1, patch_size//2)    # padding=(ps[0]//2, ps[1]//2)
...
class MultiScaleCompressionConnector(nn.Module):
    def __init__(self, c_in=1024, c_out=2048, sizes=[16,8], global_tokens=4):
        super().__init__()
        # 两个尺度的卷积分支
        self.branch1 = nn.Conv2d(c_in, c_out//2, kernel_size=3, stride=2, padding=1)
        self.branch2 = nn.Conv2d(c_in, c_out//2, kernel_size=5, stride=4, padding=2)
        self.norm1 = nn.GroupNorm(1, c_out//2)
        self.norm2 = nn.GroupNorm(1, c_out//2)
        # 可学习全局token
        self.global_tokens = nn.Parameter(torch.randn(global_tokens, c_out))
        self.fc = nn.Linear(c_out, c_out)

    def forward(self, x, text_embed=None):
        # 多尺度卷积
        b = x.shape[0]
        x1 = self.norm1(self.branch1(x)) # [B, c_out/2, H/2, W/2]
        x2 = self.norm2(self.branch2(x)) # [B, c_out/2, H/4, W/4]
        # 上采到同一尺度后拼接
        x2_up = F.interpolate(x2.float(), size=x1.shape[-2:], mode='bilinear', align_corners=False).to(x2.dtype)
        feat = torch.cat([x1, x2_up], dim=1) # [B, c_out, H/2, W/2]
        B,C,H,W = feat.shape
        tokens = feat.view(B,C,H*W).permute(0,2,1) # [B, HW, C]
        # 拼接全局token
        global_tokens = self.global_tokens.unsqueeze(0).expand(B,-1,-1)
        tokens = torch.cat([tokens, global_tokens], dim=1)
        tokens = self.fc(tokens)
        return tokens  # [B, HW+G, C]
...
def build_vision_projector(config, delay_load=False, **kwargs):
    ...
    if projector_type == 'MSCC':
        out_dim = getattr(config,'projector_outdim',4096) # 2048
        input_dim = getattr(config,'mm_input_embeds',1024) # 1024
        return MultiScaleCompressionConnector(input_dim, out_dim, sizes=[16, 8])
