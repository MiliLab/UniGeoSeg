...
from unigeoseg.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, \
...
from unigeoseg.model.mask_decoder.mask_criterion.pretrain_criterion import UniGeoSegCriterion, hungarian_matcher_PSALM
from transformers import PhiModel, PhiForCausalLM, PhiConfig

from unigeoseg.model.language_model.projector import D_Projector
...
class AttentionPooling(nn.Module):
    ...
    def forward(self, x):
        query = self.query.to(dtype=x.dtype)
        attn_scores = torch.matmul(query, x.transpose(0, 1)) / (x.size(-1)** 0.5)
...
class EnhancedAttentionPooling(nn.Module):
    def __init__(self, hidden_size, num_heads=4, dropout=0.1):
        ...
        self.multi_query = nn.Parameter(torch.randn(num_heads, hidden_size) * 0.01)

        self.functional_proj = nn.Linear(hidden_size, hidden_size)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_heads):
            query = self.multi_query[i].to(dtype=x.dtype)
            attn_scores = torch.matmul(query, x.transpose(0, 1)) / (x.size(-1) ** 0.5)
            attn_weights = torch.softmax(attn_scores, dim=-1)
            multi_attn_weights.append(attn_weights)


        multi_pooled = []
        for attn_weights in multi_attn_weights:
            pooled = torch.matmul(attn_weights, x)
            multi_pooled.append(pooled)


        functional_features = self.functional_proj(x)
        functional_pooled = torch.mean(functional_features, dim=0)


        multi_pooled_tensor = torch.stack(multi_pooled, dim=0)
        multi_pooled_mean = torch.mean(multi_pooled_tensor, dim=0)

        fused_features = torch.cat([multi_pooled_mean, functional_pooled], dim=-1)
        enhanced_pooled = self.fusion(fused_features)


        enhanced_pooled = self.dropout(enhanced_pooled)
        enhanced_pooled = self.layer_norm(enhanced_pooled)

        return enhanced_pooled


class UniGeoSegModel(LlavaMetaModel, PhiModel):
    config_class = LlavaConfig

    def __init__(self, config: PhiConfig, mask_decoder_cfg=None):
        super(UniGeoSegModel, self).__init__(config)
...
class UniGeoSeg(PhiForCausalLM, LlavaMetaForCausalLM):
    config_class = LlavaConfig

    def __init__(self, config, mask_decoder_cfg=None, add_cross_attn=True, cross_attn_index=None, use_seg_query=False):
        super(UniGeoSeg, self).__init__(config)

        self.model = UniGeoSegModel(config, mask_decoder_cfg)
        ...
        self.refer_pooling = AttentionPooling(hidden_size=config.hidden_size)
        additional_dim = 512
        ...
        self.reason_norm = nn.LayerNorm(config.hidden_size, dtype=torch.bfloat16)
        self.ref_attn = AttentionPooling(hidden_size=config.hidden_size)

        self.enhanced_reasoning_pooling = EnhancedAttentionPooling(
            hidden_size=config.hidden_size,
            num_heads=4,
            dropout=0.1
        )

        self.intera_coord_project = nn.Linear(4, config.hidden_size, dtype=torch.bfloat16)
        self.intera_fuse = nn.Linear(config.hidden_size * 2, config.hidden_size, dtype=torch.bfloat16)
        self.task_embed = nn.Embedding(3, config.hidden_size, dtype=torch.bfloat16)

        self.num_global_latents = 4

        with torch.no_grad():
            self.global_latents = nn.Parameter(torch.randn(self.num_global_latents, config.hidden_size) * 0.01)
        self.global_projector = nn.Linear(config.hidden_size, additional_dim)
        ...
        self.ablation_all_reasoning = False


        self.ablation_all_ref = False


        if self.ablation_all_reasoning and self.ablation_all_ref:
            raise ValueError("消融实验开关冲突：ablation_all_reasoning 和 ablation_all_ref 不能同时为True")


        self.task_embed = nn.Embedding(3, config.hidden_size)
    ...
    def mask_decoder_training_init(self, cfg):
        ...
        self.criterion = UniGeoSegCriterion(
            matcher=matcher,
            losses=losses,
            num_points=cfg.MODEL.MASK_FORMER.TRAIN_NUM_POINTS,
    ...
    def concat_image_seg_embeds(self, input_id, img_feature, label, seg_query, seg_query_mask,
                                    ...
                                    answer_embedding_indices=None, answer_embedding=None, token_answer_id=None, prompt_type=None, intera_coords=None):
        ...
        for chunk in chunks:
            ...
            elif chunk_len >= 1 and multitask_encoder_flag:
                base_embed = self.get_model().embed_tokens(input_id[:chunk_len])


                seq_len, hidden_size = base_embed.shape


                if self.ablation_all_reasoning:


                    task_embed = self.enhanced_reasoning_pooling(base_embed)

                    task_embed = task_embed.unsqueeze(0).expand(base_embed.size(0), -1)
                elif self.ablation_all_ref:


                    attn_weights = self.ref_attn.query @ base_embed.T / (hidden_size ** 0.5)
                    attn_weights = torch.softmax(attn_weights, dim=-1).squeeze(0).unsqueeze(-1)
                    task_embed = base_embed * attn_weights
                else:
                    ...
                    task_id = {
                        ...
                        'intera': self.TASK_INTERA
                    }[prompt_type]
                    task_code = torch.tensor([task_id], device=base_embed.device, dtype=torch.long)


                    is_ref = (task_code == self.TASK_REF).float().unsqueeze(-1).to(dtype=base_embed.dtype)
                    is_reason = (task_code == self.TASK_REASON).float().unsqueeze(-1).to(dtype=base_embed.dtype)
                    is_intera = (task_code == self.TASK_INTERA).float().unsqueeze(-1).to(dtype=base_embed.dtype)



                    reason_embed = self.enhanced_reasoning_pooling(base_embed)

                    reason_embed = reason_embed.unsqueeze(0).expand(base_embed.size(0), -1)


                    attn_weights = self.ref_attn.query @ base_embed.T / (hidden_size ** 0.5)
                    attn_weights = torch.softmax(attn_weights, dim=-1).squeeze(0).unsqueeze(-1)
                    ref_embed = base_embed * attn_weights



                    coord_tensor = intera_coords
                    device = base_embed.device
                    dtype = base_embed.dtype

                    if intera_coords is None or intera_coords.numel() == 0:

                        coord_tensor = torch.zeros((1, 4), device=device, dtype=dtype)
                    else:
                        coord_tensor = intera_coords.to(device=device, dtype=dtype)
                        if coord_tensor.dim() == 1:
                            coord_tensor = coord_tensor.unsqueeze(0)
                        if coord_tensor.size(1) == 2:
                            zeros = torch.zeros(coord_tensor.size(0), 2, device=device, dtype=dtype)
                            coord_tensor = torch.cat([coord_tensor, zeros], dim=1)



                    coord_embed = self.intera_coord_project(coord_tensor).to(dtype=base_embed.dtype)


                    coord_embed_expand = torch.zeros(seq_len, hidden_size, device=device, dtype=dtype)
                    n_points = min(coord_embed.size(0), seq_len)
                    coord_embed_expand[:n_points, :] = coord_embed[:n_points]


                    fuse_input = torch.cat([base_embed, coord_embed_expand], dim=-1)
                    intera_embed = self.intera_fuse(fuse_input)


                    task_embed = (
                        reason_embed * is_reason +
                        ref_embed * is_ref +
                        intera_embed * is_intera +
                        base_embed * (1.0 - is_reason - is_ref - is_intera)
                    )

                cur_new_input_embeds.append(task_embed)
    ...
    def prepare_inputs_labels_for_multimodal(
            ...
            token_refer_id=None, token_answer_id=None, refer_embedding_indices=None, answer_embedding_indices=None, use_seg_query=False, prompt_type=None, intera_coords=None
    ):
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if use_seg_query:
                cur_input_embeds, cur_label, cur_seg_query_mask, cur_refer_embedding_indices = self.concat_image_seg_embeds(
                    ...
                    prompt_type=prompt_type[batch_idx],
                    intera_coords=intera_coords[batch_idx] if intera_coords is not None else None
                )
    ...
    def get_SEG_embedding(self, hidden_states, refer_embedding_indices, return_all=False, prompt_type='reason'):
        ...
        for current_hidden_state, current_token_indice in zip(hidden_states, refer_embedding_indices):
            ...
            if current_refer_state.shape[0] == 0:

                current_pool_refer_state = torch.zeros(self.config.hidden_size, device=current_hidden_state.device)
            else:

                current_pool_refer_state = self.ref_attn(current_refer_state)


                global_latents = self.global_latents.to(device=current_pool_refer_state.device, dtype=current_pool_refer_state.dtype)

                similarity = torch.matmul(global_latents, current_pool_refer_state.unsqueeze(-1))
                weights = torch.softmax(similarity, dim=0)

                task_fusion_ratios = {'intera': 0.0, 'ref': 0.2, 'reason': 0.2}
                fusion_ratio = task_fusion_ratios.get(prompt_type, 0.2)

                latent_contribution = torch.sum(global_latents * weights, dim=0)

                current_pool_refer_state = (1 - fusion_ratio) * current_pool_refer_state + fusion_ratio * latent_contribution



            if return_all:

                current_pool_refer_state = torch.cat(
                    [current_pool_refer_state.unsqueeze(0), current_refer_state],
                    dim=0
                )
    ...
    def forward(
        ...
        prompt_type: Optional[List[str]] = None,
        intera_coords: Optional[List[str]] = None
        ...
        if batch_dataset_type != "mm_conv":
            input_ids, attention_mask, past_key_values, inputs_embeds, labels, seg_query_mask, refer_embedding_indices = self.prepare_inputs_labels_for_multimodal(
                ...
                token_refer_id, token_answer_id, refer_embedding_indices, answer_embedding_indices, self.use_seg_query, prompt_type, intera_coords)
        else:
            seg_query_mask = None
        ...
        if batch_dataset_type == 'refer_seg' or batch_dataset_type == 'reason_seg':
            ...
            mask_features, transformer_encoder_features, multi_scale_features = self.pixel_decoder.forward_features(
                image_features)
            if refer_embedding_indices is not None:
                SEG_embedding = self.get_SEG_embedding(hidden_states, refer_embedding_indices, return_all = True, prompt_type=prompt_type[0] if prompt_type else 'reason')
...
AutoModelForCausalLM.register(LlavaConfig, UniGeoSeg)
